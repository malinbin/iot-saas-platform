...
const menuItems = [
  ...
  { icon: FileText, label: '操作日志', href: '/admin/logs' },
...
export function AdminSidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 h-screen w-[280px] bg-[#0A1628] border-r border-[#1E3A5F]">
      {/* Logo */}
      ...
      <nav className="flex flex-col gap-1 p-4">
        <div className="mb-4 text-xs font-medium uppercase tracking-wider text-[#64748B]">
          主导航
        </div>
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <ChevronRight className="ml-auto h-4 w-4 text-[#2563EB]" />
              )}
            </Link>
          );
        })}
